...
//!      State serializes via the caller's existing Saveable machinery
//!      with no library-specific glue.
//!   2. Transitions are declared comptime. Guards and actions are
//!      `*const fn` pointers baked in at comptime — no closures, no
//!      runtime dispatch tables, no virtual calls.
//!   3. Two flavors of transition coexist in one table:
//!        - polled       (event == null)   — fired by advance(), every
//!                                            frame from the caller's tick
//!        - event-driven (event != null)   — fired by dispatch(event),
//!                                            from external signals
//!   4. Context is passed by value. Guards and actions must be pure;
//!      mutations to Context fields evaporate when the call returns,
//!      enforcing "all transient state lives on the component" at the
//!      type level.
...
//!     const Machine = fsm.StateMachine(MyState, MyEvent, MyContext);
//!     const my_machine: Machine = .{ .transitions = &.{ ... } };
//!
